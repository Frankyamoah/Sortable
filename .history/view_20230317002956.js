...
if (e.target.classList.contains('sortth')){
 ...
 const derection = e.target.dataset.darection
  ...
  case  quary === "intelligence":
   ...
   case  quary === "race":
    if(+derection === 1){
      renderHeros([...data].sort((a, b)=> a.appearance.race.toLowerCase().localeCompare( b.a.appearance.race.toLowerCase())), 1, pagenum)
      e.target.dataset.darection = -1
 } else{
  renderHeros([...data].sort((a, b) => (a.toLowerCase() > b.toLowerCase()  ? -1 : 1)), 1 ,pagenum)
  e.target.dataset.darection = 1
 } 
