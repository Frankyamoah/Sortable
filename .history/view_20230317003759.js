 ...
 const derection = e.target.dataset.darection
  ...
  e.target.dataset.darection = 1
   ...
   const sorted  = [...data].sort((a, b) => a..toLowerCase().localeCompare( b..toLowerCase()))
    const  emty = sorted.filter( el => el. === '')
    const notemty = sorted.filter( el => el.  !== "" )
    if(+derection === 1){
      renderHeros([...data].sort((a, b)=> a.appearance.gender?.toLowerCase().localeCompare( b.appearance.gender?.toLowerCase())), 1, pagenum)
      e.target.dataset.darection = -1
 } else{
