...
const table = document.querySelector('table')
 let resultsPerPageSelect = document.getElementById("results-per-page");
  const pagenum =  resultsPerPageSelect.value === 'All'? data.length -1 : resultsPerPageSelect.value
...
if (e.target.classList.contains('sortth')){
 const derection = table.dataset.darection
  ...
  if  (resultsPerPageSelect.value === 'All'){
    resultsPerPageSelect.value =  data.length -1
   }
  switch (1 < 2){
    case  quary === "name":
       if(+derection === 1){
        console.log(resultsPerPageSelect.value)
            renderHeros([...data].sort(), +resultsPerPageSelect.value)
       } else{
        renderHeros([...data].sort().reverse())
       }
