...
case quary === "placeOfBirth":
  if(+derection === 1){
const sorted  = [...data].sort((a, b) => a.iography.bplaceOfBirth.toLowerCase().localeCompare( b.iography.bplaceOfBirth.toLowerCase()))
    const  emty = sorted.filter( el => el.iography.bplaceOfBirth === "-")
    const notemty = sorted.filter( el => el.iography.bplaceOfBirth  !== "-" )
    if(+derection === 1){
      renderHeros( [...notemty, ...emty], 1, pagenum)
      e.target.dataset.darection = -1
 } else{
  renderHeros([...data].sort((a, b) => (a.iography.bplaceOfBirth?.toLowerCase() > b.iography.bplaceOfBirth?.toLowerCase()  ? -1 : 1)), 1 ,pagenum)
  e.target.dataset.darection = 1
 } 
