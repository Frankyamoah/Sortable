...
if (e.target.classList.contains('sortth')){
 ...
 const derection = e.target.dataset.darection
  ...
  case  quary === "intelligence":
   ...
   case  quary === "strength":
    if(+derection === 1){
         renderHeros(data.sort((a,b)=> parseInt( a.powerstats?.)-  parseInt( b.powerstats?.)), 1, pagenum)
         e.target.dataset.darection  = -1
    } else{
     renderHeros(data.sort((a,b)=>parseInt( b.powerstats?.)- parseInt( a.powerstats?.)), 1 ,pagenum)
     e.target.dataset.darection = 1
    }
   break;
