...
e.target.dataset.darection = 1
   ...
   case  quary === "weight":
    if(+derection === 1){
      const sorted  = [...data].sort((a, b) => a.appearance.gender.toLowerCase().localeCompare( b.appearance.gender.toLowerCase()))
      const  emty = sorted.filter( el => el.appearance.gender.include("kg"))
      const notemty = sorted.filter( el => (el.appearance.gender.include('kg')) )

     renderHeros([...data].sort((a, b)=> parseInt( a.appearance.weight[1])-  parseInt( b.appearance.weight[1]) ), 1, pagenum)
     e.target.dataset.darection = -1
 } else{
