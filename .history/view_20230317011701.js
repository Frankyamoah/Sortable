...
case quary === "placeOfBirth":
  if(+derection === 1){
const sorted  = [...data].sort((a, b) => a.a.toLowerCase().localeCompare( b.a.toLowerCase()))
    const  emty = sorted.filter( el => el.a === "-")
    const notemty = sorted.filter( el => el.a  !== "-" )
    if(+derection === 1){
      renderHeros( [...notemty, ...emty], 1, pagenum)
      e.target.dataset.darection = -1
 } else{
  renderHeros([...data].sort((a, b) => (a.a?.toLowerCase() > b.a?.toLowerCase()  ? -1 : 1)), 1 ,pagenum)
  e.target.dataset.darection = 1
 } 
