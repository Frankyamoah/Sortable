...
if (e.target.classList.contains('sortth')){
 ...
 const derection = e.target.dataset.darection
  ...
  e.target.dataset.darection = 1
   ...
   case  quary === "height":
   if(+derection === 1){
    renderHeros([...data].sort((a, b)=> parseInt( a.appearance.height[1])-  parseInt( b.appearance.height[0]) ), 1, pagenum)
    e.target.dataset.darection = -1
} else{
  renderHeros([...data].sort((a, b)=>  parseInt( b.appearance.height[0]) -  parseInt( a.appearance.height[0]) ), 1, pagenum)
e.target.dataset.darection = 1
} 
