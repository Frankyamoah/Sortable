...
export const sortHandler = (arr, renderHeros, quary)=>{
...
const table = document.querySelector('table')
 ...
 const helperPoerstas= (data, derection , e)=>{
  if(+derection === 1){
    renderHeros(data.sort((a,b)=> parseInt( a.powerstats?.intelligence)-  parseInt( b.powerstats?.intelligence)), 1, pagenum)
    e.target.dataset.darection  = -1
} else{
renderHeros(data.sort((a,b)=>parseInt( b.powerstats?.intelligence)- parseInt( a.powerstats?.intelligence)), 1 ,pagenum)
e.target.dataset.darection = 1
}
 }
...
if (e.target.classList.contains('sortth')){
 ...
 const derection = e.target.dataset.darection
  ...
  case  quary === "intelligence":
   ...
   case  quary === "strength":
    if(+derection === 1){
         renderHeros(data.sort((a,b)=> parseInt( a.powerstats?.strength)-  parseInt( b.powerstats?.strength)), 1, pagenum)
         e.target.dataset.darection  = -1
    } else{
     renderHeros(data.sort((a,b)=>parseInt( b.powerstats?.strength)- parseInt( a.powerstats?.strength)), 1 ,pagenum)
     e.target.dataset.darection = 1
    }
   break;
