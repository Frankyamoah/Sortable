...
case quary === "placeOfBirth":
  if(+derection === 1){
const sorted  = [...data].sort((a, b) => a.iography.placeOfBirth.toLowerCase().localeCompare( b.iography.placeOfBirth.toLowerCase()))
    const  emty = sorted.filter( el => el.iography.placeOfBirth === "-")
    const notemty = sorted.filter( el => el.iography.placeOfBirth  !== "-" )
    if(+derection === 1){
      renderHeros( [...notemty, ...emty], 1, pagenum)
      e.target.dataset.darection = -1
 } else{
  renderHeros([...data].sort((a, b) => (a.iography.placeOfBirth?.toLowerCase() > b.iography.placeOfBirth?.toLowerCase()  ? -1 : 1)), 1 ,pagenum)
  e.target.dataset.darection = 1
 } 
