...
if (e.target.classList.contains('sortth')){
 ...
 const derection = e.target.dataset.darection
  ...
  case  quary === "intelligence":
   ...
   case  quary === "race":
   if(+derection === 1){
    const sorted  = [...data].sort((a, b) => a.fullName.toLowerCase().localeCompare( b.fullName.toLowerCase()))
     const  emty = sorted.filter( el => el.fullName === '')
     const notemty = sorted.filter( el => el.fullName  !== "" )
     renderHeros( [...notemty, ...emty], 1, pagenum)
     e.target.dataset.darection  = -1
