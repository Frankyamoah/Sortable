...
if (e.target.classList.contains('sortth')){
 ...
 const derection = table.dataset.darection
  ...
  switch (1 < 2){
    case  quary === "name":
       if(+derection === 1){
            renderHeros([...data].sort((a, b)=> a.name.toLowerCase().localeCompare( b.name)), 1, pagenum)
            table.dataset.darection = -1
            return
       } else{
        renderHeros([...data].sort((a, b) => (a.name.toLowerCase() > bname.toLowerCase()  ? -1 : 1)), 1 ,pagenum)
        table.dataset.darection = 1
       }
      break;
      case  quary === "fullName":
