...
if (e.target.classList.contains('sortth')){
 ...
 const derection = e.target.dataset.darection
  ...
  e.target.dataset.darection = 1
   ...
   case  quary === "gender":
    if(+derection === 1){
      renderHeros([...data].sort((a, b)=> a.appearance?.toLowerCase().localeCompare( b.appearance?.toLowerCase())), 1, pagenum)
      e.target.dataset.darection = -1
 } else{
  renderHeros([...data].sort((a, b) => (a.appearance?.toLowerCase() > b.appearance?.toLowerCase()  ? -1 : 1)), 1 ,pagenum)
  e.target.dataset.darection = 1
 } 
